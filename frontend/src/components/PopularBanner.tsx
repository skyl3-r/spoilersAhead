...
export default function PopularBanner() {
    ...
    useEffect(() => {
        fetch('http://localhost:8000/api/fandoms').then((response) => {
            if (!response.ok) {
                throw new Error('Failed to fetch top fandoms')
            }
            return response.json();
        }).then((data) => setFandoms(data)).catch((error) => console.error('Error fetching fandoms:', error))
    }, []);

    return (<div className="flex flex-col bg-[#9fc2cc] p-3">
