...
export default function AddPost({ username }: { username: string }) {
  const router = useRouter();
  const [title, setTitle] = useState("");
  const [body, setBody] = useState("");
  const [selFandom, setSelFandom] = useState("");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    console.log(title, body, selFandom, trimmed);
        ...
        const response = await fetch("http://localhost:8000/api/createpost", {
          ...
          body: JSON.stringify({ username, title, body, selFandom, trimmed }),
        });
